...
public class PacketHandler extends MessageToMessageCodec<FMLProxyPacket, PacketBase>
{
	...
	public boolean registerPacket(Class<? extends PacketBase> cl)
	{
		if(packets.size() > 256)
		{
			Main.log("Packet limit exceeded in Flan's Mod packet handler by packet " + cl.getCanonicalName() + ".");
			return false;
		}
		if(packets.contains(cl))
		{
			Main.log("Tried to register " + cl.getCanonicalName() + " packet class twice.");
			return false;
		}
		if(modInitialised)
		{
			Main.log("Tried to register packet " + cl.getCanonicalName() + " after mod initialisation.");
			return false;
		}
	...
	protected void encode(ChannelHandlerContext ctx, PacketBase msg, List<Object> out) throws Exception
	{
		...
		{
			Main.log("ERROR encoding packet");
			Main.log(e.toString());
		}
	}
	...
	protected void decode(ChannelHandlerContext ctx, FMLProxyPacket msg, List<Object> out) throws Exception
	{
		...
		{
			Main.log("ERROR decoding packet");
			Main.log(e.toString());
		}
	}
	...
	{
		channels = NetworkRegistry.INSTANCE.newChannel("Erthilia", this);
		
		registerPacket(PacketSupressEnchantment.class);
		registerPacket(PacketUpdateDragonState.class);
		registerPacket(PacketOpenGui.class);
		registerPacket(PacketCosmetic.class);
		registerPacket(PacketUncraftItem.class);
		registerPacket(PacketHdv.class);
