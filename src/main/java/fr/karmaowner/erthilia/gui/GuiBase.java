...
import fr.karmaowner.erthilia.guicomponent.NotificationBox;
import fr.karmaowner.erthilia.guicomponent.NotificationBox.NotificationType;
...
import fr.karmaowner.erthilia.guicomponent.UIDialogBox;
...
public class GuiBase extends GuiScreen implements IGuiEmplacement {
	...
	protected List<GuiBase> childs = new ArrayList<GuiBase>();
	
	protected UIRect guiRect = new UIRect(new UIColor(0,0,0,0));
	
	private List<GraphicObject> components = new ArrayList<GraphicObject>();
	
	protected List<UIDialogBox> dialogBoxs = new ArrayList<UIDialogBox>();
	
	protected NotificationBox currentDisplayedNotification;
	
	private Queue<NotificationBox> notificationBoxs = new ConcurrentLinkedQueue<NotificationBox>();
	...
	{
		UIDialogBox dialogBox = new UIDialogBox(new UIButton(Type.SQUARE,"Confirmer",new ResourceLocation("erthilia","gui/pausemenu/empty_button.png"),new ResourceLocation("erthilia","gui/pausemenu/empty_button_hover.png"),true,confirmBtnCallback), new UIButton(Type.SQUARE,"Annuler",new ResourceLocation("erthilia","gui/pausemenu/empty_button.png"),new ResourceLocation("erthilia","gui/pausemenu/empty_button_hover.png"),true,cancelBtnCallback), new UIImage(new ResourceLocation("erthilia","gui/dialog_box.png")));
    ...
    {
		 for(int i = 0; i < childs.size(); i++) childs.get(i).mouseClickMove(x, y, state,timeSinceLastClick);
		 if(state == 0) 
		 {
	...
	{		
		...
		{
			GlStateManager.resetColor();
			dialogBoxs.get(i).draw(x, y,partialTicks);
		}
		GL11.glPopMatrix();
		...
		if(currentDisplayedNotification != null)
		{
			currentDisplayedNotification.draw(x, y, partialTicks);
			if(guiTicks % 20 == 0)
			{
		...
		else
		{
			...
			if(currentDisplayedNotification != null)
			{
				Minecraft.getMinecraft().getSoundHandler().playSound(PositionedSoundRecord.getRecord(new SoundEvent(new ResourceLocation(currentDisplayedNotification.getSound())), 1.0F, 1.0F));
			}
		}
	...
	public void drawHoveringText(List<String> p_146283_1_, int p_146283_2_, int p_146283_3_, FontRenderer font)
	{
