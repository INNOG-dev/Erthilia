...
public class UIScrollbarHorizontal extends GraphicObject implements IGuiClickableElement, IGuiDraggableElement {
    ...
    public void draw(int x, int y, float partialTicks)
    {
		...
		if(!isTextured())
		{
			leftArrowBtn.draw(x, y,partialTicks);
			...
			GuiUtils.drawImage(leftArrowBtn.getX() + (leftArrowBtn.getWidth()-4) / 2, leftArrowBtn.getY() + (leftArrowBtn.getHeight()-4) / 2, arrowTexture, 4, 4, (int)zLevel);
			GuiUtils.StopRotation();

			rightArrowBtn.draw(x, y,partialTicks);
			...
			GuiUtils.drawImage(rightArrowBtn.getX() + (rightArrowBtn.getWidth()-4) / 2, rightArrowBtn.getY() + (rightArrowBtn.getHeight()-4) / 2, arrowTexture, 4, 4, (int)zLevel);
			GuiUtils.StopRotation();
	...
	public void mouseMove(int x, int y) {
    	if (dragging)
        {
        	...
        	bar.setPosition((int)(x-(clickedX - previousBarX)), posY, barWidth, height);
        	
        	if(bar.getX() < this.getX())
        	{
        		bar.setPosition(posX, posY, barWidth, height);
        	}
        	if(bar.getX2() > this.getX2())
        	{
            	bar.setPosition(this.getX2()-barWidth, posY, barWidth, height);
        	}
        	
    		value = (float)(bar.getX() - posX)/ (float)(width - 5 - size * width);
    	...
    	else
    	{
        	...
        	bar.setPosition(posX + (int) (value * (width-barWidth)), posY, barWidth, height);
    	}
