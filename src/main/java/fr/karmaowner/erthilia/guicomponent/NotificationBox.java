...
public class NotificationBox extends GraphicObject implements IGuiClickableElement {
	...
	public NotificationBox setPosition(int x, int y, int width, int height)
	{
		...
		message.setPosition(ico.getX2()+4, ico.getY()+(ico.getHeight()-getMinecraft().fontRenderer.FONT_HEIGHT)/2, 0, 0);
	...
	{
		container.draw(x, y, partialTicks);
		ico.draw(x, y, partialTicks);
		tittle.draw(x, y, partialTicks);
		message.draw(x, y, partialTicks);
		bar.draw(x, y, partialTicks);
		if(closeBtn != null)closeBtn.draw(x, y, partialTicks);
		updateProgressBar();
