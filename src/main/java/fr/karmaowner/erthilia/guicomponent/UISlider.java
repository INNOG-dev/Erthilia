...
public class UISlider extends GraphicObject implements IGuiClickableElement, IGuiDraggableElement{
    ...
    {
		this.sliderRect.draw(x, y,partialTicks);
    	GuiUtils.renderCenteredText("" + denormalize(), posX + width / 2, posY + (height - (int)(getMinecraft().fontRenderer.FONT_HEIGHT * 0.8f)) / 2, 0.8f);
    	this.mouseMove(x, y);
	...
	public void mouseMove(int x, int y) {
    	...
    	GuiUtils.drawRect(posX + (int)(value * (float)(this.width - 4)), posY, 4, height,(int)zLevel,GuiUtils.gameColor,1);
        GuiUtils.drawRect(posX + (int)(value * (float)(this.width - 4)), posY, 4, height,(int)zLevel, GuiUtils.gameColor, 1);
