...
import fr.karmaowner.erthilia.gui.IGuiKeytypeElement;
...
public class UITextField extends GraphicObject implements IGuiClickableElement, IGuiKeytypeElement {
    ...
    {
        this.field_146211_a = getMinecraft().fontRenderer;
        this.inputRect = rect;
    ...
    public void draw(int x, int y, float partialTicks)
    {
    	...
    	this.drawTextBox(x, y, partialTicks);
    ...
    public boolean onLeftClick(int x, int y)
    {
        ...
        {  
            ...
            if (this.isFocused)
            {
                ...
                int l = x - (this.posX + this.inputOffsetX);

                l = (int) (l / this.font_scale);

                String s = field_146211_a.trimStringToWidth(this.type == Type.PASSWORD ? this.convertToPasswordDisplay(this.text).substring(this.lineScrollOffset) : this.text.substring(this.lineScrollOffset), getWidthText() - inputOffsetX - 4);
     ...
     * Draws the textbox
     */
    public void drawTextBox(int x, int y, float partialTicks)
    {
        if (this.isVisible())
        {
        	if(isTextured())
        	{
        		this.inputTexture.draw(x, y, partialTicks);
        	}
        	else
        	{
        		this.inputRect.draw(x, y, partialTicks);
        	}

            int i = isEnabled() ? this.textColor.toRGB() : this.disabledColor.toRGB();
            int j = this.cursorPosition - this.lineScrollOffset;
            int k = this.selectionEnd - this.lineScrollOffset;
            String s = field_146211_a.trimStringToWidth(this.type == Type.PASSWORD ? this.convertToPasswordDisplay(this.text).substring(lineScrollOffset) : this.text.substring(this.lineScrollOffset),(this.getWidthText() - this.inputOffsetX - 4));
            boolean flag = j >= 0 && j <= s.length();
            boolean flag1 = this.isFocused && this.cursorCounter / 6 % 2 == 0 && flag;
            int l = this.posX + 2 + this.inputOffsetX;
    ...
    {
        return (int) ((this.width - inputInsetX) / this.font_scale);
    }
    ...
    public void setSelectionPos(int p_146199_1_)
    {
        ...
        if (this.field_146211_a != null)
        {
            ...
            int k = this.getWidthText() - this.inputOffsetX - 4;
    ...
    public void setInputOffsetX(int offset)
    {
    	this.inputOffsetX = offset;
    }
    
    public void setInputInsetX(int inset)
    {
    	this.inputInsetX = inset;
    }
